...
RangeSlider {
    ...
    background: Item {
        ...
        Rectangle {
            x: (control.horizontal ? (control.first.implicitHandleWidth / 2)
                                     - (control.__isDiscrete ? 2 : 0) : 0)
            y: (control.horizontal ? 0 : (control.first.implicitHandleHeight
                                          / 2) - (control.__isDiscrete ? 2 : 0))
            width: parent.width - (control.horizontal ? (control.first.implicitHandleWidth
                                                         - (control.__isDiscrete ? 4 : 0)) : 0)
            height: parent.height - (control.horizontal ? 0 : (control.first.implicitHandleHeight
            ...
            Rectangle {
                ...
                width: control.horizontal ? control.second.position * parent.width
                                            - control.first.position * parent.width : 4
                height: control.horizontal ? 4 : control.second.position * parent.height
                                             - control.first.position * parent.height
                radius: Math.min(width, height) / 2
            ...
            property Repeater repeater: Repeater {
                ...
                delegate: Rectangle {
                    ...
                    color: (control.horizontal
                            && control.first.visualPosition < currentPosition
                            && control.second.visualPosition > currentPosition)
                           || (!control.horizontal
                               && control.first.visualPosition > currentPosition
                               && control.second.visualPosition
                               < currentPosition) ? control.Material.primaryHighlightedTextColor : control.Material.accentColor
